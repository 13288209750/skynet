 ...
 */
public class SerializableByMapperTest extends BaseTest {
    ...
    @Before
    public void before(){
        ISerializableUtil iSerializableUtil=new SerializableByMapper(new ObjectMapper()).setSerializationInclusion(JsonInclude.Include.NON_EMPTY);
        User user=new User();
    ...
    public void test1(){
        ...
        ISerializableUtil iSerializableUtil=new SerializableByMapper(new ObjectMapper());
        User user= null;
        try {
        ...
        try {
            users = iSerializableUtil.parseToGather(stringArray,Collection.class);
        } catch (SerializableParseException e) {
            e.printStackTrace();
        }
